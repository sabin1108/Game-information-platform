...
export async function TopNav({ isAuthenticated, variant = "default" }: TopNavProps) {
  const navState = variant === "auth" ? null : await getNavAuthState();
  const showAuthenticatedActions = isAuthenticated ?? navState?.isAuthenticated;

  return (
    <header className={`top-nav${variant !== "default" ? " top-nav--auth" : ""}`}>
      <div className="container top-nav__inner">
        ...
        {variant === "auth" ? (
          ...
          <nav className="nav-actions" aria-label="사용자 메뉴">
            ...
            {navState?.demoMode ? (
              <a className="button button--icon" href="/app" title="데모 대시보드" aria-label="데모 대시보드">
